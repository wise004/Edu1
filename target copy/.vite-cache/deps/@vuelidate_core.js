...
function createValidationResults(rules, model, key, resultsCache, path, config, instance, externalResults, siblingState) {
  ...
  result.$silentErrors = computed(() =>
    ruleKeys
      .filter(ruleKey => unref(result[ruleKey].$invalid))
      .map(ruleKey => {
...
function createMetaFields(results, nestedResults, childResults) {
  ...
  const $silentErrors = computed(() => {
    ...
    const nestedErrors = allResults.value
      .filter(result => (unref(result).$silentErrors || []).length)
...
function setValidations(_ref) {
  ...
  const results = createValidationResults(
    ...
    mergedConfig,
    instance,
    nestedExternalResults,
    state,
  );
  const nestedResults = collectNestedValidationResults(
    nestedValidators,
    nestedState,
    path,
    resultsCache,
  ...
  const { $dirty, $errors, $invalid, $anyDirty, $error, $pending, $touch, $reset, $silentErrors, $commit } = createMetaFields(
    results,
    nestedResults,
    childResults,
  );
...
function nestedValidations(_ref) {
  ...
  const childResults = computed(() =>
    childResultsKeys.value.reduce((results, key) => {
  ...
  function injectChildResultsIntoParent(results, _ref2) {
    ...
    if (
      ...
      ($scope !== CollectFlag.COLLECT_ALL && $scope !== childScope)
    )
      return;
...
function useVuelidate(validations, state) {
  ...
  const instance =
    currentVueInstance ||
    ((_getCurrentInstance = getCurrentInstance()) === null || _getCurrentInstance === void 0 ? void 0 : _getCurrentInstance.proxy);
  ...
  const { childResults, sendValidationResultsToParent, removeValidationResultsFromParent } = instance
    ? nestedValidations({
        $scope,
        instance,
      })
  ...
  if (!validations && componentOptions.validations) {
    ...
    onBeforeMount(() => {
      ...
      watch(
        () => (isFunction(rules) ? rules.call(state.value, new ComputedProxyFactory(state.value)) : rules),
        validations2 => {
  ...
  if (instance) {
    sendValidationResultsToParent.forEach(f =>
      f(validationResults, {
        $registerAs,
        $scope,
        $stopPropagation,
