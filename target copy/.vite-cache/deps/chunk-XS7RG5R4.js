...
var deprecationData$1 = {
  ...
  ['INSTANCE_ATTRS_CLASS_STYLE']: {
    message: componentName =>
      `Component <${componentName || 'Anonymous'}> has \`inheritAttrs: false\` but is relying on class/style fallthrough from parent. In Vue 3, class/style are now included in $attrs and will no longer fallthrough when inheritAttrs is false. If you are already using v-bind="$attrs" on component root it should render the same end result. If you are binding $attrs to a non-root element and expecting class/style to fallthrough on root, you will need to now manually bind them on root via :class="$attrs.class".`,
...
`,
  ...
  ['ATTR_FALSE_VALUE']: {
    message:
      name => `Attribute "${name}" with v-bind value \`false\` will render ${name}="false" instead of removing it in Vue 3. To remove the attribute, use \`null\` or \`undefined\` instead. If the usage is intended, you can disable the compat behavior and suppress this warning with:
  ...
  if (mappedName) {
    if (isArray(mappedName)) {
      ...
      mappedName.forEach(mapped => {
        ...
        if (mappedHook) {
          softAssertCompatEnabled('CUSTOM_DIR', instance, mapped, name);
      ...
      if (dir[mappedName]) {
        softAssertCompatEnabled('CUSTOM_DIR', instance, mappedName, name);
      }
...
function compatH(type, propsOrChildren, children) {
  ...
  if (l === 2 || is2ndArgArrayChildren) {
    ...
    return convertLegacySlots(
      convertLegacyDirectives(createVNode(type, convertLegacyProps(propsOrChildren, type), children), propsOrChildren),
    );
  }
}
var skipLegacyRootLevelProps = makeMap('staticStyle,staticClass,directives,model,hook');
...
function defineLegacyVNodeProperties(vnode) {
  if (
    isCompatEnabled$1('RENDER_FUNCTION', currentRenderingInstance, true) &&
    isCompatEnabled$1('PRIVATE_APIS', currentRenderingInstance, true)
  ) {
  ...
  extend(map2, {
    ...
    $options: i => {
      ...
      Object.defineProperties(res, {
        ...
        propsData: {
          get() {
            warnDeprecation$1('PRIVATE_APIS', i, '$options.propsData');
  ...
  function extendCtor(extendOptions = {}) {
    ...
    for (const key in Super.options) {
      ...
      mergeBase[key] = isArray(superValue)
        ? superValue.slice()
        : isObject(superValue)
          ? extend(/* @__PURE__ */ Object.create(null), superValue)
          : superValue;
    }
...
function applySingletonPrototype(app, Ctor) {
  ...
  for (const key of Object.getOwnPropertyNames(Ctor.prototype)) {
    if (key !== 'constructor') {
      ...
      if (enabled) {
        Object.defineProperty(app.config.globalProperties, key, Object.getOwnPropertyDescriptor(Ctor.prototype, key));
      }
    }
  }
...
function installCompatMount(app, context, render2) {
  ...
  app._createRoot = options => {
    ...
    instance.ctx._compat_mount = selectorOrEl => {
      ...
      else if (typeof MathMLElement === 'function' && container instanceof MathMLElement) namespace = 'mathml';
      if (true) {
        context.reload = () => {
...
function baseCreateRenderer(options, createHydrationFns) {
  ...
  const mountComponent = (initialVNode, container, anchor, parentComponent, parentSuspense, namespace, optimized) => {
    ...
    const instance =
      compatMountInstance || (initialVNode.component = createComponentInstance(initialVNode, parentComponent, parentSuspense));
  ...
  if (isCompatEnabled$1('INSTANCE_ATTRS_CLASS_STYLE', instance) && vnode.shapeFlag & 4 && root.shapeFlag & (1 | 6)) {
    ...
    if (cls || style) {
      if (inheritAttrs === false) {
        warnDeprecation$1('INSTANCE_ATTRS_CLASS_STYLE', instance, getComponentName(instance.type));
      }
      root = cloneVNode(
        root,
        {
...
function convertLegacyComponent(comp, instance) {
  ...
  if (isObject(comp) && comp.functional && softAssertCompatEnabled('COMPONENT_FUNCTIONAL', instance, comp)) {
...
function finishComponentSetup(instance, isSSR, skipOptions) {
  ...
  if (!instance.render) {
    if (!isSSR && compile$1 && !Component.render) {
      const template =
        (instance.vnode.props && instance.vnode.props['inline-template']) ||
        Component.template ||
...
function compatCoerceAttr(el, key, value, instance = null) {
  if (isEnumeratedAttr(key)) {
    ...
    if (v2CoercedValue && compatUtils.softAssertCompatEnabled('ATTR_ENUMERATED_COERCION', instance, key, value, v2CoercedValue)) {
      el.setAttribute(key, v2CoercedValue);
      return true;
    }
  } else if (value === false && !isSpecialBooleanAttr(key) && compatUtils.isCompatEnabled('ATTR_FALSE_VALUE', instance)) {
    compatUtils.warnDeprecation('ATTR_FALSE_VALUE', instance, key);
    el.removeAttribute(key);
    return true;
  }
  return false;
  ...
  if (value === '' || value == null) {
    ...
    if (value === false && compatUtils.isCompatEnabled('ATTR_FALSE_VALUE', parentComponent)) {
      ...
      if (type === 'string' || type === 'number') {
        compatUtils.warnDeprecation('ATTR_FALSE_VALUE', parentComponent, key);
        value = type === 'number' ? 0 : '';
        needRemove = true;
      }
    }
...
var withKeys = (fn, modifiers) => {
  ...
  let instance = null;
  {
    ...
    if (modifiers.some(m => /^\d+$/.test(m))) {
      compatUtils.warnDeprecation('V_ON_KEYCODE_MODIFIER', instance);
    }
  }
    ...
    if (!isFunction(component) && !component.render && !component.template) {
      ...
      if (container.nodeType === 1) {
        for (let i = 0; i < container.attributes.length; i++) {
          const attr = container.attributes[i];
          if (attr.name !== 'v-cloak' && /^(v-|:|@)/.test(attr.name)) {
            compatUtils.warnDeprecation('GLOBAL_MOUNT_CONTAINER', null);
            break;
          }
        }
      }
...
var tokenizer = new Tokenizer(stack, {
  ...
  onattribnameend(end) {
    ...
    if (currentOpenTag.props.some(p2 => (p2.type === 7 ? p2.rawName : p2.name) === name)) {
      emitError(2, start);
    }
  },
      ...
      if (quote !== 0) {
        ...
        if (currentProp.type === 6) {
          ...
          currentProp.value = {
            ...
            loc:
              quote === 1 ? getLoc(currentAttrStartIndex, currentAttrEndIndex) : getLoc(currentAttrStartIndex - 1, currentAttrEndIndex + 1),
          };
          if (
            tokenizer.inSFCRoot &&
          ...
          if (
            ...
            checkCompatEnabled('COMPILER_V_BIND_SYNC', currentOptions, currentProp.loc, currentProp.rawName)
          ) {
...
function onCloseTag(el, end, isImplied = false) {
  ...
  }
  {
    ...
    const inlineTemplateProp = props.find(p2 => p2.type === 6 && p2.name === 'inline-template');
    if (
      inlineTemplateProp &&
    ...
    if (p2.type === 6) {
      if (p2.name === 'is' && p2.value) {
        if (p2.value.content.startsWith('vue:')) {
          return true;
        } else if (checkCompatEnabled('COMPILER_IS_ON_ELEMENT', currentOptions, p2.loc)) {
          return true;
        }
      }
    } else if (
      ...
      if (!inPre) {
        if (isAllWhitespace(node.content)) {
          ...
          if (
            ...
            !next ||
            (shouldCondense &&
              ((prev === 3 && (next === 3 || next === 1)) || (prev === 1 && (next === 3 || (next === 1 && hasNewlineChar(node.content))))))
          ) {
...
function walk(node, parent, context, doNotHoistNode = false, inFor = false) {
  ...
  function getSlotNode(node2, name) {
    if (node2.children && !isArray(node2.children) && node2.children.type === 15) {
      const slot = node2.children.properties.find(p2 => p2.key === name || p2.key.content === name);
...
function createTransformContext(
  ...
  const context = {
    ...
    cache(exp, isVNode2 = false, inVOnce = false) {
      const cacheExp = createCacheExpression(context.cached.length, exp, isVNode2, inVOnce);
...
function createCodegenNodeForBranch(branch, keyIndex, context) {
  if (branch.condition) {
    return createConditionalExpression(
      ...
      createCallExpression(context.helper(CREATE_COMMENT), [true ? '"v-if"' : '""', 'true']),
    );
  } else {
...
var transformFor = createStructuralDirectiveTransform('for', (node, dir, context) => {
  ...
  return processFor(node, dir, context, forNode => {
    ...
    let keyExp =
      keyProp && (keyProp.type === 6 ? (keyProp.value ? createSimpleExpression(keyProp.value.content, true) : void 0) : keyProp.exp);
...
function buildSlots(node, context, buildSlotFn = buildClientSlotFn) {
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    if ((vIf = findDir(slotElement, 'if'))) {
      hasDynamicSlots = true;
      dynamicSlots.push(
        createConditionalExpression(vIf.exp, buildDynamicSlot(slotName, slotFunction, conditionalBranchIndex++), defaultFallback),
      );
    } else if (
      (vElse = findDir(
        slotElement,
...
var transformElement = (node, context) => {
  return function postTransformElement() {
    ...
    if (props.length > 0) {
      ...
      vnodeDirectives =
        directives && directives.length ? createArrayExpression(directives.map(dir => buildDirectiveArgs(dir, context))) : void 0;
...
function buildProps(node, context, props = node.props, isComponent2, isDynamicComponent, ssr = false) {
  ...
  for (let i = 0; i < props.length; i++) {
    ...
    if (prop.type === 6) {
      ...
      if (
        name === 'is' &&
        (isComponentTag(tag) || (value && value.content.startsWith('vue:')) || isCompatEnabled('COMPILER_IS_ON_ELEMENT', context))
      ) {
        continue;
      }
      properties.push(
      ...
      if (
        name === 'is' ||
        (isVBind && isStaticArgOf(arg, 'is') && (isComponentTag(tag) || isCompatEnabled('COMPILER_IS_ON_ELEMENT', context)))
      ) {
        continue;
      }
      if (isVOn && ssr) {
...
var DOMNodeTransforms = [transformStyle, ...(true ? [transformTransition, validateHtmlNesting] : [])];
...
@vue/compat/dist/vue.esm-bundler.js:
  ...
  **)
  (*! #__NO_SIDE_EFFECTS__ *)
*/
//# sourceMappingURL=chunk-XS7RG5R4.js.map
