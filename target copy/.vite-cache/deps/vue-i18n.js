import { setupDevtoolsPlugin } from './chunk-GZOPNIPP.js';
import {
  Fragment,
  Text,
  computed,
...
if (true) {
  ...
  if (
    ...
    perf.mark &&
    perf.measure &&
    perf.clearMarks && // @ts-ignore browser compat
    perf.clearMeasures
  ) {
    mark = tag => {
      perf.mark(tag);
...
var friendlyJSONstringify = json =>
  JSON.stringify(json)
    .replace(/\u2028/g, '\\u2028')
    .replace(/\u2029/g, '\\u2029')
    .replace(/\u0027/g, '\\u0027');
...
function datetime(context, ...args) {
  ...
  for (let i = 0; i < locales.length; i++) {
    ...
    if (isPlainObject(format3)) break;
...
function createMessageContext(options = {}) {
  ...
  const pluralRule =
    isObject(options.pluralRules) && isString(locale) && isFunction(options.pluralRules[locale])
      ...
      : pluralDefault;
  const orgPluralRule =
    isObject(options.pluralRules) && isString(locale) && isFunction(options.pluralRules[locale]) ? pluralDefault : void 0;
  ...
  const normalize =
    isPlainObject(options.processor) && isFunction(options.processor.normalize) ? options.processor.normalize : DEFAULT_NORMALIZE;
  const interpolate =
...
function translate(context, ...args) {
  ...
  let [formatScope, targetLocale, message] = !resolvedMessage
    ...
    : [key, locale, messages[locale] || create()];
  ...
  if (isString(format3) && context.messageCompiler == null) {
    warn(
      `The message format compilation is not supported in this build. Because message compiler isn't included. You need to pre-compilation all message format. So translate function return '${key}'.`,
    );
    return key;
  }
  let occurred = false;
    ...
    if (!key.includes('.')) {
      ...
      for (let i = 0; i < lastIndex; i++) {
        ...
        if (!isObject(currentObj[subKeys[i]])) {
          warn(
            getWarnMessage2(I18nWarnCodes.IGNORE_OBJ_FLATTEN, {
              key: subKeys[i],
            }),
          );
          hasStringValue = true;
    ...
    if (
      ...
      if (__root && isString(key) && isResolvedTranslateMessage(warnType, arg2)) {
        if (_fallbackRoot && (isTranslateFallbackWarn(_fallbackWarn, key) || isTranslateMissingWarn(_missingWarn, key))) {
          warn(
            getWarnMessage2(I18nWarnCodes.FALLBACK_TO_ROOT, {
              key,
              type: warnType,
            }),
          );
...
function editScope(payload, i18n) {
  ...
  if (composer) {
    ...
    if (field === 'locale' && isString(payload.state.value)) {
      ...
      (isString(payload.state.value) || isArray(payload.state.value) || isObject(payload.state.value))
    ) {
...
function convertComposerOptions(options) {
  ...
  const missingWarn =
    isBoolean(options.silentTranslationWarn) || isRegExp(options.silentTranslationWarn) ? !options.silentTranslationWarn : true;
